...
### Added
- **Notifications** — a new `NOTIFY_URL` setting POSTs a summary of each finished sync to a webhook, so a failed scan no longer goes unnoticed until someone opens the dashboard. Discord, Slack, Gotify and ntfy are detected from the URL and formatted natively (Discord gets a colour-coded embed, Gotify/ntfy get raised priority on failure); any other URL receives a structured JSON POST carrying the full result — status, path, episode, quality, profile, custom formats, duration and error. `NOTIFY_ON` selects `error` (failures only, the default), `all`, or `off`. Delivery runs on a background thread so a slow or unreachable webhook never delays a scan, and the Settings page has a **Save & Send Test** button that saves and then fires a test notification through the stored URL.
...
### Changed
- **Release sync-back now uses merge commit** — the post-release CHANGELOG sync from master back to dev now uses `git merge --no-ff` instead of a file copy, making the release commit an ancestor of dev and preventing 3-way merge collisions on subsequent dev→master PRs.
