...
### Fixed
- **compose.yaml env vars silently overriding the Settings page** — most `environment:` entries used `${VAR:-default}`, so even when a value was never set in `.env`, Docker Compose baked the hardcoded default into the container's real environment. The app then saw that as "set via environment" and permanently locked the field in the Settings page, regardless of whether the user had actually configured anything. These now pass through as `${VAR}` (no fallback) so leaving a value unset in `.env` correctly leaves it editable in Settings.
